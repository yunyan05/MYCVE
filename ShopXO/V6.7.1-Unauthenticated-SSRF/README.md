...
- The SSRF primitive itself is demonstrated end-to-end below: an unauthenticated remote attacker can coerce the ShopXO server into issuing arbitrary outbound HTTP/HTTPS requests, confirmed out-of-band via DNS resolution from the server's egress IPs. Based on source-level analysis of `extend/base/Uploader.php`, `app/common.php::CurlGet` and `app/service/UeditorService.php::ActionCrawler`, the same primitive additionally allows requests to the loopback interface, link-local cloud metadata service (`169.254.169.254`) and other reserved ranges, because the IP filter uses only `FILTER_FLAG_NO_PRIV_RANGE`; and because the fetched response body is persisted under `/static/upload/...` with its URL returned in the JSON response, the primitive is read-out rather than blind in source. The realised impact in a given deployment therefore ranges from blind internal-network reachability (proven here) to read-out of internal HTTP responses and cloud metadata (analysed from the code path, not demonstrated in this submission).

# DESCRIPTION

- During the security assessment of ShopXO V6.7.1 (release 2025-10-28), I identified an unauthenticated Server-Side Request Forgery vulnerability in `extend/base/Uploader.php::saveRemote()`. The vendor introduced an IP filter in the 6.5.x branch to address the historical CVE-2024-6524 (`source` parameter SSRF, fixed-version annotation `<=6.1.0`), but the fix uses only `FILTER_FLAG_NO_PRIV_RANGE` and therefore — based on the documented behaviour of that PHP filter flag and the source of `extend/base/Uploader.php` — leaves loopback, link-local and other reserved ranges reachable. The crawler entry points are exposed through both the public front-end (`/index.php?s=index/ueditor/index`) and the API (`/index.php?s=api/ueditor/index`) without any login requirement, and the downstream cURL transport sets `CURLOPT_FOLLOWLOCATION=true` without re-validating the redirect target. The SSRF primitive itself is demonstrated end-to-end in this submission via an out-of-band DNS canary; the further consequences described below (loopback / cloud-metadata reachability, read-out via the persisted attachment URL) are derived from the source code path and are not separately demonstrated here. Immediate corrective actions are still warranted because the source-level analysis shows the same primitive can be redirected to internal targets without additional vendor-side guards.
...
The SSRF primitive is demonstrated here at the "server-side fetch is issued to an attacker-supplied host" level via the DNSLog canary. Based on the source code, repeating the request with `source[0]=http://127.0.0.1:<port>/...` (or, on cloud-hosted deployments, `http://169.254.169.254/latest/meta-data/...`) is expected to follow the same code path because `FILTER_FLAG_NO_RES_RANGE` is not set; the persisted attachment URL in the JSON response is the source-level read-out channel. These further consequences are derived from the source code and are not separately reproduced in this submission.
